...
function generarPDF($id)
{
    ...
    $config = include_once 'app/config/api_keys.php';
    $db = AccesoDatos::getModelo();
    $cli = $db->getCliente($id);
    ...
    ob_start();
    require "app/views/pdf.php"; // Asegúrate de que esta vista genere el HTML necesario
    $html = ob_get_clean();

    // Configuración de la API
    $apiKey = $config['HTML2PDF_API_KEY'];
    $apiEndpoint = "https://api.html2pdfrocket.com/pdf";

    // Parámetros para la API
    $postData = [
        "apikey" => $apiKey,  // Tu API Key
        "value" => $html,     // Contenido HTML
        "FileName" => "cliente_$id.pdf" // Nombre del archivo
    ];
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($postData)); // Datos en formato URL
    ...
    if ($httpCode == 200) {
        // Configurar cabeceras para mostrar el PDF en el navegador
...
function recuperarImagenCliente($id)
{
    ...
    $rutaImagenDefault = "app/uploads/default.png";
    ...
    if (file_exists($rutaImagenJPG)) {
        ...
        $imagenBase64 = 'data:image/png;base64,' . base64_encode(file_get_contents($rutaImagenDefault));;
    }
